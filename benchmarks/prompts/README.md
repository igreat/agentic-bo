...
- `oer_naive.md`: use inside a `skill_profile=bo_only` benchmark workspace with plain Claude Code and no explicit skill invocation

## HER open-world case study

- `her_live_structural_naive.md`: no orchestration hint
- `her_live_structural_light.md`: soft workflow hint in the initial prompt, but no explicit `/research-agent`
- `her_live_structural_strong.md`: use after explicitly invoking `/research-agent`

For the HER core pair, use fresh clean repo workspaces from the same branch or commit and keep the ordinary project skill trees present in both workspaces. The only intended intervention difference is the explicit skill invocation.
