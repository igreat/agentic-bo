...
**Length:** 2000–2500 words, excluding title and abstract.
...
## Introduction
- state the research or optimization problem and why it matters
- summarise only relevant literature context if available
- motivate why optimization is needed instead of exhaustive search

If literature review was skipped:
- keep the introduction minimal and artifact-scoped
- do not add unsupported historical or benchmark claims
- state plainly that no literature baseline comparison was performed for this run

## Methods / Methodology
- describe the search space actually used
- describe the BO engine and relevant configuration
- state how observations were obtained based on the artifacts
- describe oracle provenance only from what the artifacts explicitly say
- if proxy-backed:
  - report oracle / surrogate RMSE if available
  - clearly state that outcomes reflect predictions rather than direct measurements
- reference the convergence plot path if present

## Results
- report the best value found and corresponding candidate
- mention convergence behaviour qualitatively or quantitatively depending on evidence
- use `report.json` as the source of truth for top-level best-value summary statistics
- if `report.json["trajectory"]` is present, prefer it for phase summaries
- if exact iteration-level claims are needed and not in `report.json`, verify them from `observations.jsonl`
- if exact support is missing, keep the results narrative qualitative

## Discussion
- interpret the result in domain terms only if supported by the artifacts or literature inputs
- compare against literature baselines if available
...
If literature review was skipped:
- keep the discussion grounded in:
  - BO trajectory
  - candidate composition
  - oracle uncertainty
  - simulation limitations
- label any mechanism-style interpretation as tentative and artifact-derived

## Conclusion / Why it matters
- summarise what was found
- state one practical next step
- for reports, include a short "Why it matters" statement when useful
...
- ✓ Label proxy/surrogate results as **simulated** throughout
- ✓ Extract facts directly from JSON artifacts
- ✓ Use `report.json` as source of truth for best value and iteration count
- ✓ Include oracle/surrogate RMSE if available in `oracle_meta.json`
- ✓ State constraints and search-space limits explicitly
- ✗ Do not present surrogate predictions as laboratory measurements
- ✗ Do not invent numeric details unsupported by artifacts
- ✗ Do not add generic BO boilerplate
