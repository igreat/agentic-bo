...
from bo_workflow.observers.base import Observer
from bo_workflow.infrastructure.oracle import _normalize_objective_values
...
from .engine_helpers import detect_smiles_column, infer_design_parameters


def _detect_smiles_column(df: pd.DataFrame, hint: str | None = None) -> str:
    return detect_smiles_column(df, hint=hint)
...
def _infer_design_parameters(
    ...
    *,
    ...
    return infer_design_parameters(frame, max_categories=max_categories)
...
class BOEngine:
    ...
    def init_molecular_run(
        ...
        *,
        ...
        """
        from bo_workflow.molecular.scaffold import load_scaffold_spec, spec_to_design_parameters
        from bo_workflow.molecular.features import get_descriptor_feature_names
    ...
    def _expand_smiles_direct_features(
        ...
        energy_fn = None
        if energy_backend != "none":
            from bo_workflow.molecular.energy import get_energy_features
    ...
    def _load_molecular_spec(self, state: dict[str, Any]) -> Any:
        ...
        from bo_workflow.molecular.scaffold import load_scaffold_spec
    ...
    def _load_energy_cache(self, run_id: str, state: dict[str, Any]) -> Any:
        ...
        """
        from bo_workflow.molecular.energy import EnergyCache
    ...
    def _expand_molecular_features(
        ...
        """
        from bo_workflow.molecular.features import expand_features_for_oracle
    ...
    def build_oracle(
        ...
        *,
        ...
        if state.get("mode") == "molecular":
            ...
            if energy_backend != "none":
                from bo_workflow.molecular.energy import get_energy_features
                from bo_workflow.molecular.scaffold import decode_suggestion
    ...
    def _build_optimizer(
        ...
        if observations:
            x_rows = [
                {
                    k: row.get("x", {}).get(k, 0.0)
                    for k in state["active_features"]
                }
    ...
    def suggest(
        ...
        *,
        ...
        if state.get("mode") == "molecular":
            from bo_workflow.molecular.feasibility import assess_feasibility_batch
            from bo_workflow.molecular.scaffold import decode_suggestion
    ...
    def evaluate_last_suggestions(
        ...
        from bo_workflow.infrastructure.oracle import predict_original_scale
        y_pred = predict_original_scale(self._paths(run_id).run_dir, state, x_df)
    ...
    def run_proxy_optimization(
        ...
        *,
        ...
        When *observer* is ``None`` (default), uses the built-in
        ``evaluate_last_suggestions`` which correctly handles both
        standard and SMILES-discovery modes.
        """
        use_builtin = observer is None
        label = "proxy-oracle (builtin)" if use_builtin else observer.source
        self._log(
            verbose,
            f"[run] run_id={run_id} iterations={num_iterations} batch_size={batch_size} observer={label}",
        )
        progress = tqdm(
        ...
        for _ in progress:
            ...
            if use_builtin:
                self.evaluate_last_suggestions(run_id, verbose=verbose)
            else:
    ...
    def init_scaffold_edit_run(
        ...
        *,
        ...
        # --- Seed selection ---
        from bo_workflow.molecular.crem_ops import (
        ...
        from bo_workflow.molecular.energy import _estimate_strain_energy
        from bo_workflow.molecular.feasibility import assess_feasibility
